...
except ImportError:  # NVDA >= 2021.1.
...
def errcheck(res, func, args):
	if res != 0:
		raise RuntimeError(f"{func.__name__}: code {res}")
	return res
...
class SynthDriver(synthDriverHandler.SynthDriver):
	...
	def __init__(self):
		...
		self.dectalk.TextToSpeechOpenInMemory.errcheck = errcheck
		self.dectalk.TextToSpeechGetCaps.errcheck = errcheck
		...
		self.dectalk.TextToSpeechStartup(self._messageWindow, byref(self.handle), -1, DO_NOT_USE_AUDIO_DEVICE)
		caps = TTS_CAPS_T()
		...
		self.dectalk.TextToSpeechOpenInMemory(self.handle, FORMAT)
		self.dectalk.TextToSpeechAddBuffer(self.handle, byref(self.mem_buffer))
	...
	def handle_buffer(self, lpBuffer):
		...
		self.dectalk.TextToSpeechAddBuffer(self.handle, lpBuffer)

	def speak(self, speechSequence):
		textList = [
			"[:skip rule",  # Kills some abbrevs.
			":comma 0",  # Reset pause after commas.
			":comma -280",  # Reduce pause after commas.
			":period 0",  # Reset pause after periods.
			":period -420",  # Reduce pause after periods.
			":mode spell off",
			":punctuation none",  # NVDA will handle punctuation.
...
#				item = item.replace("[:", " ")
		...
		self.dectalk.TextToSpeechSpeak(self.handle, " ".join(textList).encode("iso8859-1", "ignore"), 1)

	def cancel(self):
		# TextToSpeechReset CAN SOMETIMES cause an NVDA core freeze.
		# self.dectalk.TextToSpeechReset(self.handle, 0)
	...
	def terminate(self):
		self.cancel()
		self.dectalk.TextToSpeechShutdown(self.handle)
	...
	def _set_rate(self, rate):
		...
		self.dt_rate = val
		self.dectalk.TextToSpeechSpeak(self.handle, b"[:rate %d]" % val, 1)
	...
	def _set_voice(self, val):
		...
		self._voice = val
		self.dectalk.TextToSpeechSpeak(self.handle, f"[:name {val[0].lower()}]".encode("latin-1"), 1)
