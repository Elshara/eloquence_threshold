...
When you plan a new migration sprint, review the complementary [`assets/md/assets_migration_backlog.md`](assets_migration_backlog.md). The backlog lists every framework that still lives partially under `speechdata/`, highlights which cached datasets need suffix audits, and prescribes the smoke tests (`python -m unittest discover assets/py 'test_*.py'`) plus offline packaging drill (`python build.py --insecure --no-download --output dist/eloquence.nvda-addon`) we expect before publishing changes for NVDA alpha-52731 and CodeQL review. Add `--trace-json assets/json/build_execution_trace.json --trace-markdown assets/md/build_execution_trace.md` when you package so reviewers can confirm which helpers ran, which optional directories were skipped, and which follow-ups to prioritise in the modernization audit. Update the table after each move so we know which payloads remain in motion and which ones are ready for packaging.
...
When you need a complete binary versus text breakdown of `speechdata/`, run `python assets/py/report_binary_assets.py --json assets/json/binary_asset_index.json --markdown assets/md/binary_asset_index.md --print`. The helper indexes every engine directory, flags DLL/EXE/SYN/UIL archives, and links them back to the NVDA loader modules that still depend on those binaries. Pair the report with [`assets/md/binary_asset_audit.md`](binary_asset_audit.md) so reviewers can see both the strategic remediation notes and the per-file inventory before rehearsing offline packaging runs.
...
8. **Validate the output** – open the generated `.nvda-addon` (it is a ZIP archive) to confirm the `manifest.ini`, voice catalogues, and `assets/` runtime folders (especially `speechdata/eloquence/dll/` and `speechdata/eloquence/syn/`) are present. Cross-check the `docs/` excerpts packaged inside if you enabled documentation bundling and ensure any legacy `eloquence*/` folders you staged for compatibility also made it into the archive.
...
| 3. Stage binaries | Populate `speechdata/eloquence/dll/` with the `eci*.dll` runtime files and `speechdata/eloquence/syn/` with the corresponding `.syn` voices (optionally mirror them in `eloquence*/` folders for legacy workflows). | Provide architecture-matched runtimes; the builder validates PE headers and fails fast on mismatches. |
