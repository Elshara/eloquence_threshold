...
class SynthDriver(SynthDriver):
	...
	@classmethod
	def check(cls):
		return os.path.isfile(os.path.join(file_path, 'newfon_nvda.dll'))
	...
	def __init__(self):
		...
		self.hasDictLib = os.path.isfile(os.path.join(file_path, 'dict.dll'))
		if self.hasDictLib:
			self.sdrvxpdb_lib = windll.LoadLibrary(os.path.join(file_path, 'sdrvxpdb.dll'))
			self.dict_lib = windll.LoadLibrary(os.path.join(file_path, 'dict.dll'))
		self.newfon_lib = windll.LoadLibrary(os.path.join(file_path, 'newfon_nvda.dll'))
	...
	def speakText(self, text, index=None):
		...
		text = text.encode('cp1251', errors='replace')
