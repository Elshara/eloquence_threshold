...
4. Use the new metadata summaries (`summaries.extensions`, `summaries.sample_rates`, `summaries.languages`, `summaries.language_tags`, `summaries.synth_hints`, `summaries.families`, `summaries.voice_hints`, `summaries.platforms`, `summaries.versions`, `summaries.categories`, and `summaries.viability`) in `docs/archive_inventory.json` to queue phoneme imports, language template work, and cross-platform tooling audits before mirroring lower-priority demo audio.
